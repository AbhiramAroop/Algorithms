...
class HashTable:
    def __init__(self, table_capacity=None, hash_base=None):
        ...
        if hash_base == None:
            hash_base = 31
        self.table = [None] * table_capacity #creating table
    ...
    def __getitem__(self, key=None):
        ...
        for i in range(0,self.table_size):
            if self.table[pos] == None: #No item in position?
                raise KeyError
            elif self.table[pos][0] == key: #correct item in position
                return self.table[pos][1]
            else:
                pos = (pos+1)%self.table_size #collision
    ...
    def __setitem__(self, key, item):
        ...
        j = 1
        for i in range(0, self.table_size):
            if probe > self.probe_max: #update max
                    self.probe_max = probe 
            if self.table[pos] == None: #Empty position?
                self.table[pos] = (key,item)
                self.count += 1
                return
            elif self.table[pos][0] == key: #same key already there?
                self.table[pos] =(key,item)
                return
            else: #collision?
                if collision == False: 
    ...
    def rehash(self):
        ...
        for prime in Primes: #Checks correct prime value to choose
        ...
        for i in range(0,old_size): #transfer old values
...
def table_load_dictionary_statistics(max_time):
    ...
    print(data)
    with open('output_task4.csv','w',encoding='utf-8') as csvFile: #stores the computational times as a list and adds it to a csv file
