...
class ListADT:
    ...
    def __eq__(self,other):
        ...
        if self.length >= other.__len__(): #if self.length has greater or equal length
            ...
            for i in range(0,other.len()): #checks elements with other.len()
                if self.the_array[i] != other.__getitem__(i):  #same item?
                    return False     
            for i in range(other.len(),len(other)): #other elements can only be None
                if other.__getitem__(i) != None: #same item?
                    return False 
        return True
